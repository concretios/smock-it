...
import { saveEmailLocally, loadEmailLocally } from '../../localStorage.js';
import { askQuestion } from './template/init.js';

const VERCEL_API_URL = 'https://smockit-user-validator.vercel.app/api/validate-user';
const SIGNUP_LINK = `\u001b]8;;${'https://mailchi.mp/concretio/smock-it-promptify'}\u0007${'Signup here'}\u001b]8;;\u0007`;
...
export default class TemplateGenerate extends SfCommand<void> {
  ...
  public async checkUserEmail(email: string): Promise<boolean> {
    try {
      const resp = await axios.get(`${VERCEL_API_URL}?email=${encodeURIComponent(email)}`);
      return resp.data.allowed === true;
    } catch {
  ...
  public async run(): Promise<void> {
    try {
      let email = loadEmailLocally();
      if (!email) {
        // eslint-disable-next-line @typescript-eslint/restrict-template-expressions
        email = await askQuestion(`Enter your registered email id to access this command (${chalk.blueBright.underline(SIGNUP_LINK)}):`);
        // eslint-disable-next-line no-useless-escape
        if (!email || !/^[\w\.-]+@[\w\.-]+\.\w+$/.test(email)) {
          this.error('Invalid email format.');
        }
      }
      const allowed = await this.checkUserEmail(email.trim().toLowerCase());
      if (!allowed) {
        this.log(
          '\n' +
            chalk.red('Access Denied:') +
            ' Your email is not registered with us.\n' +
            `To request access, please ${chalk.blueBright.underline(SIGNUP_LINK)} ` +
            '\n\nOnce your email is approved, you can rerun the command.'
        );
        return;
      }
      if (!loadEmailLocally()) {
        saveEmailLocally(email.trim().toLowerCase());
      }
      ...
      if (!openAiToken) {
        ...
        if (!openAiToken) {
          // Prompt the user to enter their OpenAI key IF THE IS NOT SET IN ENVIRONMENT VARIABLE
          openAiToken = await this.promptForOpenAiKey();
        }
        // Set the validated OpenAI key as an environment variable
        process.env.OPENAI_KEY = openAiToken;
      }
      // Validate the token before any further steps
      await this.validateOpenAIKey(openAiToken);

      // Ask the prompt for data generation and Salesforce org alias
      ...
      // Connect to the Salesforce org and validates
    ...
    try {
      ...
      const answers = await Enquirer.prompt<Answers>({
        ...
        message: `Select output format ${chalk.dim(
          `(use ${chalk.cyanBright('<space>')} to select, ${chalk.cyanBright('↑')} ${chalk.cyanBright(
            '↓'
          )} to navigate)`
        )}:`,
  ...
  private async validateOpenAIKey(apiKey: string): Promise<boolean> {
    try {
      // Make a GET request to the OpenAI models endpoint
  ...
  private async setUserEnvironmentVariable(name: string, value: string): Promise<void> {
    // Before setting the environment variable, validate the key
    await this.validateOpenAIKey(value);
    // Determine the OS and execute the appropriate command
    const platform = os.platform();
    if (!platform) {
      throw new Error('Unable to determine the operating system.');
    }
  ...
  private async promptForOpenAiKey(): Promise<string> {
    // Ask the user to input their OpenAI key
  ...
  private async getOpenAIKeyFromUserEnv(): Promise<string | undefined> {
    ...
    return new Promise((resolve) => {
      exec(command, (error, stdout) => {
        ...
        const key = platform === 'win32' ? stdout.match(/OPENAI_KEY\s+REG_SZ\s+(.+)/)?.[1]?.trim() : stdout.trim();
  ...
  private async generateTemplate(
    userPrompt: string,
    outputFormats: string[] = ['di', 'json', 'csv'],
    openAiToken: string
  ): Promise<{ templateName: string; templateJson: any }> {
    ...
    // Generate the template using OpenAI API
...
Based on the user prompt, generate the JSON template strictly following these rules. Ensure that fields identified as dependent picklists are included only with the "dp-" prefix and their regular versions are excluded. **Double-check that field values are assigned correctly**, applying values only to the last field in ambiguous phrases (e.g., "Field1, Field2 set to X" means only Field2 gets X) unless specified otherwise.
    ...
    for (const sObject of templateJson.sObjects) {
      ...
      // Remove regular fields for which dependent picklists exist
      ...
      // Handle if the same fields are present in both regular and dependent picklist forms
      ...
      // Special handling for case and contact objects
      if (
        objectKey === 'case' &&
  ...
  private async handleDataGeneration(templateName: string, usernameOrAlias: string): Promise<void> {
    console.log(
      chalk.blueBright('Template Path: ') +
        chalk.yellowBright(path.join(process.cwd(), 'data_gen', 'templates', templateName))
    );
    console.log(
      chalk.blueBright(
        'You can customize the template located in the data_gen/templates folder prior to generating data.'
      ) + '\n'
    );
    ...
    // Run the command to generate data using the template and the specified Salesforce org alias
