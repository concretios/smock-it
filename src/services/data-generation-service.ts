...
/**
 * DataGenerationService orchestrates the end-to-end workflow for generating mock Salesforce data,
 * Connect to a Salesforce org using provided alias, process the configuration file, 
 * enhancing it with special fields, saving the output in desired formats (JSON/CSV), and optionally
 * inserting it into a connected Salesforce org.
 **/
...
import { DataGenerateResult, tempValidateFlags, GenericRecord, jsonConfig } from '../utils/types.js';
...
import {
  processObjectConfiguration,
  generateFieldsAndWriteConfig,
  processObjectFieldsForIntitalJsonFile,
  enhanceDataWithSpecialFields,
...
export class DataGenerationService {
  ...
  private flags: tempValidateFlags;

  public constructor(flags: tempValidateFlags) {
    this.flags = flags;
  ...
  public async execute(): Promise<DataGenerateResult> {
    ...
    const objectsToProcess = processObjectConfiguration(baseConfig, this.flags.sObjects);
    ...
    const jsonDataForObjectNames: jsonConfig = JSON.parse(
      await fs.promises.readFile(configPath, 'utf-8')
    ) as jsonConfig;
    const outputFormat: string[] = jsonDataForObjectNames.outputFormat ?? [];
    const sObjectNames: string[] = jsonDataForObjectNames.sObjects.map(
      (sObject: { sObject: string }) => sObject.sObject
    );
    if (!sObjectNames) {
    ...
    for (const object of sObjectNames) {
      ...
      if (!fields) {
        console.log(`No fields found for object: ${object}`);
        continue;
      }
      ...
      const { failedCount: failedInsertions } = await SalesforceConnector.handleDirectInsert(
        conn,
        outputFormat,
        object,
